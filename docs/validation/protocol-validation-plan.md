...
A `pcm-audio` sender test SHOULD demonstrate which source event established
each new stream anchor. If the source exposes the first-sample acquisition
...
materialization fallback MUST NOT be applied to V/I records.
